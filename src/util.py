...
def ddi_rate_score(record, dataset='mimic3', path=None):
    if path is None:
        path = f'../data/{dataset}/output/ddi_A_final.pkl'
    # ddi rate
...
def parameter_report(best, regular, savepath):
    """输出一个参数的文件"""
    with open(os.path.join(savepath,"parameter_report.txt"), 'w+') as f:
        f.write(
...
def graph_report(history, savepath):
    """图像方式输出结果"""
    ...
    plt.legend()
    plt.savefig(os.path.join(savepath, 'jaccard.png'))
