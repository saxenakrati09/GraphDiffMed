...
def eval_one_epoch(model, data_eval, voc_size, args):
    ...
    for step, input_seq in enumerate(data_eval):
        ...
        for adm_idx, adm in enumerate(input_seq):
            output, _ = model(input_seq[:adm_idx + 1], args)
    ...
    ddi_rate = ddi_rate_score(smm_record, dataset=args.dataset)
...
def Test(model, device, data_test, voc_size, args):
    ...
    for _ in range(10):
        ...
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med, avg_fp, avg_fn \
            = eval_one_epoch(model, test_sample, voc_size, args)
...
def Train(model, device, data_train, data_eval, voc_size, args):
    ic(args)
    if not os.path.exists(os.path.join(args.savepath, args.dataset)): # args.savepath = data/saved/args.dataset
        os.makedirs(os.path.join(args.savepath, args.dataset))
    ...
    for epoch in range(EPOCH):
        ...
        for step, input_seq in enumerate(data_train):
            for adm_idx, adm in enumerate(input_seq):
                ...
                result, loss_ddi = model(input_seq[:adm_idx + 1], args)
                ...
                current_ddi_rate = ddi_rate_score([[y_label]], dataset=args.dataset)
        ...
        ddi_rate, ja, prauc, avg_p, avg_r, avg_f1, avg_med, avg_fp, avg_fn = eval_one_epoch(model, data_eval, voc_size, args)
        ...
        graph_report(history, os.path.join(args.savepath, args.dataset))
        parameter_report(best, regular, os.path.join(args.savepath, args.dataset))

    print('avg training time/epoch: {:.4f}'.format(total_train_time / EPOCH))
    parameter_report(best, regular, os.path.join(args.savepath, args.dataset))

    torch.save(best['model'].state_dict(), os.path.join(args.savepath, args.dataset, "trained_model_{:.4f}".format(best['ja'])))
    return best['model']
